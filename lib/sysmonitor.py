...
class SystemMonitor:
    def __init__(self, cameraserver = None):
    ...
    def ChangeMode(self, value=""):
        ...
        if ret:
            msg = ["Mode changed", "Mode : {0}".format(str(eMode(self.GetMode())).lstrip('eMode.')), "OK"]
            start_new_thread(self.DrawMsg, ([msg[0], msg[1], ""],))
        else:
            msg = ["Mode unchanged", "Mode : {0}".format(str(eMode(self.GetMode())).lstrip('eMode.')), "ERROR"]
    ...
    def Run(self):
        try:
            ...
            while self.running:
                ...
                if mode == eMode.CAMERA.value:
                    if not self.GetCameraRunning():
                        start_new_thread(self.RunCamera, (draw,))
                else:
                    if not self.GetMsgRunning():
                        if mode == eMode.CLOCK.value:
                            self.DrawClock(draw)
                        elif mode == eMode.INFO.value:
                            self.DrawInfo(draw)
